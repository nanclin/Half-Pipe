...
public class BezierSpline : MonoBehaviour
{
	...
	private BezierSpline _incomingSpline;
	private BezierSpline _outgoingSpline;
	public BezierSpline incomingSpline;
	public BezierSpline outgoingSpline {
		get{
			return _outgoingSpline;
		}
		set{
			_outgoingSpline = value;
			value.incomingSpline = this;
		}
	}
...
//////////////////////////////////////////////////////////// EO GET POINT //
	...
	private void EnforceMode( int index )
	{
		...
		Vector3[] chainPoints = points;

		// Skip enforcing for FREE points and START/END points
		// if( mode == BezierPointMode.Free || modeIndex == 0 || modeIndex == modes.Length - 1 )
		if( mode == BezierPointMode.Free )
			return;
		else if( modeIndex == 0 ){
			if( incomingSpline != null ){
			}
			else
				return;
		}
		else if( modeIndex == modes.Length - 1 ){
			if( outgoingSpline != null ){
				print("anclin: " + outgoingSpline);
				Array.Resize( ref chainPoints, chainPoints.Length + outgoingSpline.points.Length );
				outgoingSpline.points.CopyTo( chainPoints, outgoingSpline.points.Length );
			}
			else
				return;
		}
		...
		Vector3 point = chainPoints[ pointIndex ];
		Vector3 enforcedTangent = point - chainPoints[ holdingIndex ];
		// Keep length of enforced anchor
		// in case of Aligned mode
		if( mode == BezierPointMode.Aligned )
			enforcedTangent = enforcedTangent.normalized * Vector3.Distance( point, chainPoints[ enforcedIndex ] );
		chainPoints[ enforcedIndex ] = point + enforcedTangent;
	}
